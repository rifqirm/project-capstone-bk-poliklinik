      ...
      <div class="w-full max-w-full px-3 mt-0 mb-6">
        <div
          ...
          <form action="{{ route('admin-updatePasien',  $pasien->id) }}" method="POST"
            ...
            @method('PUT')
            <div class="mb-4">
              <label for="nama" class="block text-sm font-medium text-gray-700 mb-1" style="margin: 0.5rem;">Nama
                Pasien:</label>
            ...
            <div class="mb-4">
              <label for="alamat" class="block text-sm font-medium text-gray-700 mb-1"
                style="margin: 0.5rem;">Alamat:</label>
            ...
            <div class="mb-4">
              <label for="no_hp" class="block text-sm font-medium text-gray-700 mb-1"
                style="margin: 0.5rem;">No.HP:</label>
              <input type="text" name="no_hp" class="w-full p-2 border rounded-md"
                style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;"
                value="{{ $pasien->no_hp }}" required>
            </div>
            <div class="mb-4">
              <label for="no_ktp" class="block text-sm font-medium text-gray-700 mb-1"
                style="margin: 0.5rem;">No.KTP:</label>
              <input type="text" name="no_ktp" class="w-full p-2 border rounded-md"
                style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;"
                value="{{ $pasien->no_ktp }}" required>
            </div>
            <div class="mb-4">
              <label for="no_rm" class="block text-sm font-medium text-gray-700 mb-1"
                style="margin: 0.5rem;">No.Rm:</label>
              <div class="relative">
                <p class="w-full p-2 border rounded-md"
                  style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;">Nomor
                  Rekam Medis akan diisi secara otomatis oleh sistem setelah pasien melakukan registrasi akun!</p>
              </div>
              <div class="mb-4 flex justify-center">
                <button type="submit"
                  class="btn btn-success shadow-soft-2xl rounded-lg bg-dark stroke-0 text-center xl:p-2.5"
                  style="background-image: linear-gradient(to bottom right, #00A9FF, #11fc00); color:white;">Simpan
                  Perubahan</button>
              </div>
          </form>
