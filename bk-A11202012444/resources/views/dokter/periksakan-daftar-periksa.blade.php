        ...
        <div class="w-full max-w-full px-3 mt-0 mb-6">
          <div
            ...
            <div class="flex-auto p-6 px-0 pb-2">
                      ...
                      <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">
                        <div class="flex px-2 py-1">
                          <div class="flex flex-col justify-center">
                            <center>
                              <h6 class="mb-0 text-sm leading-normal">{{ $pasienIni->pasien->nama }}</h6>
                            </center>
                          </div>
                        </div>
                      </td>
                      <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">{{
                        $pasienIni->jadwalPeriksa->hari
                        }},{{$pasienIni->jadwalPeriksa->jam_mulai}}-{{$pasienIni->jadwalPeriksa->jam_selesai}}</td>
                      <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">{{ $pasienIni->keluhan }}
                      </td>
                      <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">{{ $pasienIni->no_antrian
                        }}</td>
                    </tr>
                  </tbody>
                </table>
              ...
              <div class="overflow-x-auto">
                <form action="{{ route('dokter-simpan-Periksakan-Daftar-Periksa', ['id' => $pasienIni->id]) }}"
                  method="POST" class="bg-white p-8 rounded-lg shadow-md">
                  @csrf
                  <div class="mb-4">
                    <label for="catatan" class="block text-sm font-medium text-gray-700 mb-1"
                      style="margin: 0.5rem;">Catatan:</label>
                    <input type="text" name="catatan" class="w-full p-2 border rounded-md"
                      style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;">
                  </div>
                  <div class="mb-4" hidden>
                    <label for="biaya_periksa" class="block text-sm font-medium text-gray-700 mb-1"
                      style="margin: 0.5rem;">Biaya Periksa:</label>
                    <input type="text" name="biaya_periksa" class="w-full p-2 border rounded-md"
                      style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;"
                      readonly value="150000">
                  </div>
                  <div class="mb-4">
                    <label for="obat" class="block text-sm font-medium text-gray-700 mb-1"
                      style="margin: 0.5rem;">Obat:</label>
                    <div id="obatContainer">
                      <select id="obatSelect" class="w-full p-2 border rounded-md"
                        style="width: calc(100% - 1rem); padding: 0.5rem; border-radius: 0.375rem; margin: 0.5rem;">
